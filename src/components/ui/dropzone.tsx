...
import { fileAtom } from "@/state/atoms/fileUploader"; 
...
export function FileUploader(props: FileUploaderProps) {
  ...
  const [files, setFiles] = useAtom(fileAtom); // Use the atom here
      ...
      >
        {({ getRootProps, getInputProps, isDragActive }) => (
          <div
            {...getRootProps()}
            className={cn(
              "group relative grid aspect-video w-full cursor-pointer place-items-center rounded-lg border-2 border-dashed border-muted-foreground/25 px-5 py-2.5 text-center transition hover:bg-muted/25",
              ...
              <div className="flex flex-col items-center justify-center gap-4 sm:px-5">
                ...
                <div className="flex flex-col gap-px">
                  ...
                  <p className="text-sm text-muted-foreground/70">
                    You can upload
                    {maxFileCount > 1
                      ? ` ${maxFileCount === Infinity ? "multiple" : maxFileCount} files (up to ${formatBytes(maxSize)} each)`
